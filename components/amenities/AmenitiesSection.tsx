...
export function AmenitiesSection({ content, amenities }: AmenitiesSectionProps) {
  ...
  const [cardStep, setCardStep] = useState(0); // Initialize with 0
  ...
  const GAP = 16; // Shared carousel gap (matches the gap-4 below)

  // Measure desktop card step
  ...
  const goForwardDesktop = () => {
    if (canGoForwardDesktop) setCurrentIndex((prev) => prev + 1);
  };

  // Mobile carousel logic (mimicking TestimonialsSection)
  const mobileMaxIndex = Math.max(0, itemCount - 1); // 1 card visible at a time
      ...
      <ScrollReveal from="bottom" delay={0}>
        <div className="flex justify-between items-center w-full max-w-[1920px] mx-auto px-5 sm:px-10 lg:px-[80px] mb-8 xl:mb-[100px]">
          ...
          <div className="flex gap-3 items-center flex-shrink-0">
            <button
              onClick={isMobile ? goBackMobile : goBackDesktop}
              disabled={isMobile ? false : !canGoBackDesktop} // Always enabled for mobile wrapping
              ...
              style={{ opacity: isMobile || canGoBackDesktop ? 1 : 0.35 }}
            ...
            <button
              onClick={isMobile ? goForwardMobile : goForwardDesktop}
              disabled={isMobile ? false : !canGoForwardDesktop} // Always enabled for mobile wrapping
              className="btn-circle-arrow transition-all duration-200 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-[#265090]"
              style={{ opacity: isMobile || canGoForwardDesktop ? 1 : 0.35 }}
