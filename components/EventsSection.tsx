...
type WhatIncludesItem = { text: string; icon: string };
...
export default function EventsSection({ content }: EventsSectionProps) {
  const FALLBACK_ITEMS: WhatIncludesItem[] = [
  ...
  return (
    <main data-header-theme="light" className="w-full bg-brand-bg font-sans">
      {/* Hero Video */}
      <div className="w-full h-[220px] sm:h-[320px] md:h-[420px] lg:h-[520px] xl:h-[600px] overflow-hidden">
        <video
          poster="https://cdn.builder.io/api/v1/image/assets%2F1e8c7b62118c49ceb84fba99d4c44471%2F44e9d92b87a543418f5003c61070b2df?format=webp&width=1920"
          autoPlay
          loop
          muted
          playsInline
      ...
      <section className="bg-brand-bg px-5 sm:px-8 lg:px-0">
        <div className="max-w-[1920px] mx-auto lg:px-[80px] xl:px-[80px]">
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-0 py-12 lg:py-16 xl:py-20">

            {/* Left label column */}
            <div className="hidden lg:flex lg:w-[200px] xl:w-[240px] flex-shrink-0 pt-1">
              <span className="text-brand-navy text-[11px] font-medium uppercase tracking-[2.4px] leading-[1.2]">
                Private Events
              </span>
            </div>

            {/* Main content column */}
            <div className="flex-1 lg:max-w-[560px] xl:max-w-[620px] lg:pr-8 xl:pr-12">
              {/* Mobile label */}
              <span className="lg:hidden block text-brand-navy text-[11px] font-medium uppercase tracking-[2.4px] leading-[1.2] mb-4">
                Private Events
              </span>

              <ScrollReveal>
                <h2
                  className="text-brand-navy text-[20px] sm:text-4xl xl:text-[40px] uppercase mb-4 lg:mb-6"
              ...
              <ScrollReveal delay={100}>
                <p className="text-black text-lg sm:text-xl xl:text-2xl font-normal leading-[1.3] mb-4 lg:mb-5 max-w-[500px]">
                  {content.description}
                </p>

                <p className="text-brand-gray text-sm sm:text-base font-normal leading-6 max-w-[320px] mb-8 lg:mb-10">
                  {/* Additional description text - keeping empty as per original */}
                </p>
              </ScrollReveal>
            ...
            {/* Right panel */}
            <div className="lg:flex-1 lg:max-w-[720px]">
              <ScrollReveal delay={200}>
                <div className="bg-brand-panel p-8 sm:p-10 xl:p-12 flex flex-col gap-4 h-full">

                {/* What includes label */}
                <span className="text-brand-gray text-[11px] font-medium uppercase tracking-[2.4px] leading-[1.2]">
                  What Includes
                </span>
                  ...
                  {whatIncludes.map((item, idx) => (
                    <div key={idx} className="flex flex-col gap-2">
                      <div className="flex items-center justify-between gap-4">
                        <span className="text-black text-xs sm:text-sm font-extrabold uppercase tracking-[0.7px] leading-[1.2] flex-1">
                          {item.text}
                        </span>
                ...
                {/* Footer text */}
                <p className="text-brand-gray text-sm sm:text-base font-normal leading-6 mt-1">
                  With premium amenities and tailored experiences, we'll make your event unforgettable.
                </p>
              </div>
              </ScrollReveal>
            </div>
...
function SpikeletLeft() {
  return (
    <svg width="19" height="48" viewBox="0 0 19 48" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
      ...
      <defs>
        <clipPath id="spikelet-left-clip">
          <rect width="19" height="48" fill="white" transform="matrix(-1 0 0 1 19 0)"/>
        </clipPath>
      </defs>
    </svg>
  );
...
function SpikeletRight() {
  return (
    <svg width="19" height="48" viewBox="0 0 19 48" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
      <g clipPath="url(#spikelet-right-clip)">
      ...
      <defs>
        <clipPath id="spikelet-right-clip">
