...
/** Fallback amenities used when WP returns no amenities for a location */
const FALLBACK_AMENITIES = [
  { label: "12 courts",     iconName: "courts" },
  { label: "Lounge Zones",  iconName: "lounge" },
  { label: "Coworkings",    iconName: "coworking" },
  { label: "Pro-Shop",      iconName: "proshop" },
  { label: "Cafe",          iconName: "cafe" },
  { label: "Fitness Areas", iconName: "fitness" },
];
...
export default function LocationsSection({ content, locations: locationsProp }: LocationsSectionProps) {
  ...
  const ctaTitle = getButtonTitle(ctaText);
  ...
  /** Resolve the icon ReactNode for an amenity, preferring inline icon then iconName map */
  const resolveIcon = (amenity: { icon?: ReactNode; iconName?: string }): ReactNode => {
    const key = (amenity.iconName ?? '').toLowerCase().trim();
    return amenity.icon ?? LOCATION_ICON_MAP[key] ?? null;
  };
                    ...
                    {selected.amenities.map((amenity, i) => (
                      <div key={i}>
                        <div className="flex items-center gap-8 px-2.5">
                          <span className="flex-shrink-0">{resolveIcon(amenity)}</span>
